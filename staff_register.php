...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		$sql = "SELECT * FROM staff_users WHERE demail='$email'";
		$result = mysqli_query($db, $sql);

		
		
		if (!$result->num_rows > 0) {
			$sql = "INSERT INTO staff_users (did, demail, dpassword,position)
					VALUES ('$cnic', '$email', '$password','$desg')";
			$result = mysqli_query($db, $sql);
			

			$profile = "INSERT INTO staff_profile (did,first_name,last_name,gender,city,country,birth_date,age,specialization,department,designation) VALUES('$cnic','$fname_e','$lname_e','$gender_e','$city_e','$country_e','$dob_e','$age_e',' $spec_e',' $dep_e','$desg')";
